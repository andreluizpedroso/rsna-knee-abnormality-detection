# Notebook histórico (linha híbrida: RadImageNet + LLM, publicScore 0.911)

⚠️ **Não é mais a linha ativa.** Substituída em 2026-08-17 pela linha
DINO-RadImageNet Rank Ensemble (`notebooks/kaggle_ativo/`,
`publicScore = 0.920`). Mantida como histórico/fallback, mesmo padrão
já usado com `notebooks/kaggle_ativo_v15_tta/`.

Este é o snapshot versionado do notebook Kaggle que gerou o melhor
`publicScore` real da competição até então. Substituiu a linha anterior
(v15 + TTA, `notebooks/kaggle_ativo_v15_tta/`, `publicScore = 0.899`) em
2026-08-17.
