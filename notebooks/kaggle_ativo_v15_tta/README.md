# Notebook histórico (linha Pilkwang v15 + TTA, superada em 2026-08-17)

**⚠️ Esta pasta não é mais a linha ativa.** Foi substituída em 2026-08-17
pela linha híbrida (RadImageNet + LLM), `publicScore = 0.911` contra o
`0.899` daqui — ver `notebooks/kaggle_ativo/`. Mantida como registro
histórico e caminho de volta caso a linha nova precise ser revertida (ver
branch git `backup/v15-tta-0899`, criado antes da troca).

Este é o snapshot versionado do notebook Kaggle que gerava o melhor
`publicScore` real da competição até 2026-08-17. Antes de existir aqui,
ele só existia nos servidores do Kaggle e numa pasta temporária do
sistema (fora do git) — puxado/editado/empurrado via `kaggle kernels
pull`/`push` direto na conta do Kaggle.
